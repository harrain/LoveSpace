...
public abstract class AbstractTaskWorker {	
	...
	private final Executor getExecutor(Task task) {
		...
		return com.example.lovespace.common.infra.TaskExecutor.IMMEDIATE_EXECUTOR;
	}
	
	private final Runnable getRunnable(final com.example.lovespace.common.infra.Task task) {
		return new Runnable() {
			@Override
			public void run() {
				// schedule
